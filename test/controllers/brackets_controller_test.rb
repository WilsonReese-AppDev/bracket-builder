...
class BracketsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bracket" do
    assert_difference('Bracket.count') do
      post brackets_url, params: { bracket: { number_of_entries: @bracket.number_of_entries, status: @bracket.status } }
    end
  ...
  test "should update bracket" do
    patch bracket_url(@bracket), params: { bracket: { number_of_entries: @bracket.number_of_entries, status: @bracket.status } }
